...
diag_log ( "[TMW] Starting Trader City Safezone Commander!" );
...
if ( AGN_safeZoneMessages ) then { systemChat ( "[TMW] Trader Zone Commander Loaded!" ); };
...
while {true} do {
	...
	//when player enters safezone
		if (AGN_safeZoneAntispam )then{
			if (AGN_enteredSafezone) then{
				if ( AGN_safeZoneMessages ) then { systemChat ("[TMW] Antispam - Please wait before re-entering!"); };
			}else{
				AGN_enteredSafezone = true;//player has entered safezone
				if ( AGN_safeZoneMessages ) then { systemChat ("[TMW] Entering Trader Area - God Mode Enabled"); };
				if ( AGN_safeZoneMessages ) then { systemChat ("[TMW] Antispam - You must wait 2 minutes for god mode to reactivate once you leave!");};
				player_zombieCheck = {};
	...
	if ( AGN_safeZone_Players_DisableWeaponFiring ) then
	{
		_EH_Fired = _thePlayer addEventHandler ["Fired", {
			systemChat ("[TMW] You can not fire your weapon in a Trader City Area");
			...
			while {!canBuild} do
			{
				...
				if( !isNull (FindDisplay 106) && !_skip ) then
				{
					if ( isNull AGN_LastPlayerLookedAt ) then
					{
						...
						if ( AGN_safeZoneMessages ) then { systemChat ("[TMW] Anti Backpack Stealing - Redirecting you to your own gear!"); };
						waitUntil { isNull (FindDisplay 106) };
					} else {
						if ( AGN_safeZoneMessages ) then { systemChat (format["[TMW] You cannot open your gear at this time as you have looked at a player in the last 5 seconds."]); };
						(findDisplay 106) closeDisplay 1;
						waitUntil { isNull (FindDisplay 106) };
					};
				};
				if ( _skip && _if ) then {
					if ( AGN_safeZoneMessages ) then { systemChat ("[TMW] This player is tagged friendly, you have access to this players bag") };
					sleep 30;
				};
			};
		};
	...
	if ( AGN_safeZone_Vehicles_DisableMountedGuns ) then
	{
		while { !canBuild } do
		{
			...
			{
				if (AGN_safeZoneMessages) then { systemChat ( "[TMW] No Firing Vehicle Guns Enabled" ); };
				_inVehicle = vehicle player;
				_inVehicleDamage = getDammage _inVehicle;
				_EH_Fired_Vehicle = _inVehicle addEventHandler ["Fired", {
					systemChat ("[TMW] You can not fire your vehicles weapon in a Trader City Area");
					NearestObject [_this select 0,_this select 4] setPos[0,0,0];
				}];
			};
		};
	...
	if ( AGN_safeZoneMessages ) then { systemChat ("[TMW] Exiting Trader Area"); };//removed godmode disabled text
	
	if ( AGN_safeZone_Vehicles_DisableMountedGuns ) then
	{
		if ( !(isNull _inVehicle) ) then
		{
			if ( AGN_safeZoneMessages ) then { systemChat ( "[TMW] No Firing Vehicle Guns Disabled" ); };
			_inVehicle removeEventHandler ["Fired", _EH_Fired_Vehicle];
		};
